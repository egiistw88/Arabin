...
import { CheckCircle2, Lock, Play, Star, BookOpen, Trophy, Flame } from 'lucide-react';
...
export const LessonPath: React.FC<LessonPathProps> = ({ progress }) => {
  ...
  // 1. Determine which lesson is currently "Active" (The first one NOT completed)
  let activeLessonIndex = LESSON_DATA.findIndex(l => !progress.completedLessons.includes(l.id));
  
  // Handle "All Complete" scenario safely
  const isCourseComplete = activeLessonIndex === -1 && progress.completedLessons.length > 0;
  
  if (isCourseComplete) {
    activeLessonIndex = LESSON_DATA.length - 1; // Point to last lesson for review
  }
  // If nothing started (fresh), it's 0
  if (activeLessonIndex === -1) activeLessonIndex = 0;

  const activeLesson = LESSON_DATA[activeLessonIndex];
  
  // Real stats from progress
  const stats = {
    vocabMastered: progress.completedLessons.length * 6, // Estimate
    ...
    <div className="w-full min-h-screen bg-[#f8f9fa] pb-32">
      ...
      <div className="bg-white px-6 pt-8 pb-6 border-b border-gray-100 rounded-b-3xl shadow-sm z-10 relative">
        <div className="max-w-md mx-auto">
          <div className="flex justify-between items-center mb-6">
            ...
            <div className="w-10 h-10 rounded-full bg-[#1a1512] flex items-center justify-center text-[#fdfbf7] font-serif font-bold border-2 border-[#8a1c1c]">
              A
            </div>
          </div>
      ...
      <div className="max-w-md mx-auto px-6 mt-6">
        ...
        <motion.div 
          ...
          className="relative overflow-hidden bg-[#1a1512] rounded-2xl p-6 shadow-xl shadow-gray-200 mb-10 group"
        >
          ...
          <div className="relative z-10">
            <div className="flex justify-between items-start mb-4">
               <span className={`px-2 py-1 text-white text-[10px] font-bold uppercase rounded tracking-wider ${isCourseComplete ? 'bg-green-600' : 'bg-[#8a1c1c]'}`}>
            ...
            <Link 
              ...
              className="block w-full py-3 bg-[#fdfbf7] text-[#1a1512] font-bold text-center rounded-xl hover:bg-white transition-colors"
            >
          ...
          {LESSON_DATA.map((lesson, idx) => {
            ...
            return (
              ...
              >
                {/* Node Marker */}
                <div className={`
                    w-6 h-6 rounded-full border-4 flex-shrink-0 bg-white z-10 mt-6 box-content transition-colors duration-300
                    ${isCurrent ? 'border-[#8a1c1c] shadow-[0_0_0_4px_rgba(138,28,28,0.2)]' : 
                      isCompleted ? 'border-[#1a1512] bg-[#1a1512]' : 'border-gray-200'}
                `}>
                    {isCompleted && <CheckCircle2 className="w-full h-full text-white p-0.5" />}
                </div>

                {/* Lesson Content Card */}
                {/* Prevent link click if locked */}
                <Link 
                  to={!isLocked ? `/read/${lesson.id}/0` : '#'}
                  className={`
                    flex-1 bg-white rounded-xl p-5 border transition-all duration-300 block
                    ${isCurrent ? 'border-[#8a1c1c] card-shadow scale-[1.02]' : 'border-gray-100 shadow-sm hover:border-gray-300'}
                    ${isLocked ? 'cursor-not-allowed bg-gray-50' : 'cursor-pointer'}
                  `}
                >
                  <div className="flex justify-between items-start mb-2">
                     <span className={`font-sans text-[10px] font-bold uppercase tracking-wider ${isLocked ? 'text-gray-300' : 'text-gray-400'}`}>
                       Bab {idx + 1}
                     </span>
                     {isLocked && <Lock className="w-3 h-3 text-gray-300" />}
                  </div>

                  <h4 className={`font-serif font-bold text-lg mb-2 ${isCurrent ? 'text-[#8a1c1c]' : isLocked ? 'text-gray-400' : 'text-[#1a1512]'}`}>
                    {lesson.title.split(':')[1]?.trim() || lesson.title}
                  </h4>
                  
                  <div className="flex flex-wrap gap-1.5">
                    {lesson.concepts.slice(0, 3).map((concept, i) => (
                      <span key={i} className={`px-2 py-1 text-[10px] rounded-md font-medium border ${isLocked ? 'bg-transparent text-gray-300 border-gray-100' : 'bg-gray-50 text-gray-600 border-gray-100'}`}>
                        {concept}
                      </span>
                    ))}
                  </div>
